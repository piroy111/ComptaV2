...
public class CFLoanCost {

	/*
	 * Data static
	 */
	private static Map<Integer, Map<String, Double>> pMapDateToMapMetalToCostLoan = new HashMap<Integer, Map<String, Double>>();
	...
	private static void readFileConf() {
		...
		LitUnFichierEnLignes lReadFile = new LitUnFichierEnLignes(StaticDir.getCONF_COMPTA(), 
				StaticNames.getCONF_COST_LOAN(), true);
		for (List<String> lLineStr : lReadFile.getmContenuFichierListe()) {
			...
			pMapDateToMapMetalToCostLoan.put(lDateMax, lMapMetalToCostLoan);
	...
	public static final double getpCostLoan(String _sMetal, int _sDate) {
		 ...
		 * Find cost of loan
		 */
		double lCostLoan = pMapDateToMapMetalToCostLoan.get(lDateMax).get(_sMetal);
		return lCostLoan;
	}
